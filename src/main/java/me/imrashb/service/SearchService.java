...
public class SearchService {
    ...
    public List<CoursSearchResult> searchBM25(List<CoursDataWrapper> courses, String query, int limit) {
        ...
        for (DocumentData docData : documentData) {
            ...
            if (score > 0) {
                CoursSearchResult result = new CoursSearchResult(
                        docData.course.getSigle(),
                        docData.course.getTitre(),
                        docData.course.getDescription(),
                        docData.course.getCredits(),
                        score
                );
                results.add(result);
            }
    ...
    public List<CoursSearchResult> searchFuzzy(List<CoursDataWrapper> courses, String query, int limit, Integer maxDistance) {
        ...
        String queryNormalized = query.trim().toUpperCase();
        ...
        for (CoursDataWrapper course : courses) {
            ...
            if (course.getSigle() != null) {
                String sigleUpper = course.getSigle().toUpperCase();
            ...
            if (course.getTitre() != null) {
                String titreUpper = course.getTitre().toUpperCase();
                // Check if query matches any word in title
                String[] titleWords = titreUpper.split("\\s+");
                for (String word : titleWords) {
                    if (word.length() >= 3) { // Only check words of 3+ chars
            ...
            if (bestScore < 0.5 && course.getDescription() != null) {
                String descUpper = course.getDescription().toUpperCase();
                String[] descWords = descUpper.split("\\s+");
                for (String word : descWords) {
                    if (word.length() >= 4) { // Only check longer words in description
                        double wordScore = calculateFuzzyScore(queryNormalized, word, effectiveMaxDistance);
        ...
        return matches.stream()
                ...
                .map(match -> {
                    CoursSearchResult result = new CoursSearchResult();
                    result.setSigle(match.course.getSigle());
                    result.setTitre(match.course.getTitre());
                    result.setDescription(match.course.getDescription());
                    result.setCredits(match.course.getCredits());
    ...
    private List<CoursSearchResult> mergeResults(List<CoursSearchResult> bm25Results, 
        ...
        for (CoursSearchResult result : fuzzyResults) {
            String sigle = result.getSigle();
            if (sigle != null) {
                double fuzzyScore = result.getScore() != null ? result.getScore() : 0.0;
                fuzzyScore *= 0.4; // Fuzzy weight
                
                if (mergedMap.containsKey(sigle)) {
                    // Combine scores: BM25 (60%) + Fuzzy (40%)
                    CoursSearchResult existing = mergedMap.get(sigle);
                    double combinedScore = existing.getScore() + fuzzyScore;
